...
class ExampleRoutableParent_TestGeneric(
    BaseView_TestGeneric[BaseRequestBody, BaseResponse]
):
    NAME_MODULE = "Test"


class BaseViewParent_TestGeneric(
    BaseView_TestGeneric[TReq1, TRes1], Generic[TReq1, TRes1]
):
    pass


class ExampleRoutableParent1_TestGeneric(
    BaseViewParent_TestGeneric[CloneBaseRequestBody, CloneBaseResponse]
):
    pass
...
class BV_Middle_Generic1(
    BV_Generic[TReqGen3, TReqGen4, TResGen3], Generic[TReqGen3, TReqGen4, TResGen3]
):
    pass


class BVParent_Generic2(BV_Middle_Generic1[List[int], int, Dict[str, MyTestClass]]):
    pass


class BVParent_Generic3(BV_Middle_Generic1[List[int], str, MyTestClass]):
    pass
...
def is_equal_types(real_val, true_val) -> bool:
    if hasattr(real_val, ORIGIN) and hasattr(true_val, ORIGIN):
        return getattr(real_val, ORIGIN) == getattr(true_val, ORIGIN) and getattr(
            real_val, ARGS
        ) == getattr(true_val, ARGS)

    return (
        real_val is true_val
...
def test_generic__generic_types() -> None:
    ...
    true_route = {
        "/bv-parent-generic2/v1.0/convert": TrueRoute(
            RequestType=List[int], ResponseType=List[Dict[str, MyTestClass]]
        ),
        "/bv-parent-generic3/v1.0/get-list/{x}": TrueRoute(
            RequestType=str, ResponseType=List[MyTestClass]
        ),
        "/bv-parent-generic3/v1.0/test/{x}": TrueRoute(
            RequestType=str, ResponseType=str
        ),
    }
    for path, type_ in true_route.items():
        ...
        request_type = (
            None if app_routes[0].body_field is None else app_routes[0].body_field.type_
        )
