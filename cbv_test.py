...
# Begin Setup
from class_based_fastapi.new.view import cbv
...
class ItemCBV(Parent):
    # Step 3: Add dependencies as class attributes
    session: Session = Depends(get_db)
    user_id: UserID = Depends(get_jwt_user)

    @router.post("/item")
    def create_item(self, item: ItemCreate, db: Session = Depends(get_db)) -> ItemInDB:
        # Step 4: Use `self.<dependency_name>` to access shared dependencies
        item_orm = ItemORM(name=item.name, owner=self.user_id)
        self.session.add(item_orm)
        self.session.commit()
...
uvicorn.run(app, host="localhost", port=9090, debug=True)
